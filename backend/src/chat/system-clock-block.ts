...
export function formatSystemClockBlock(nowMs: number = Date.now()): string {
  ...
  return (
    ...
    '3. **仅当**下方「可用工具列表」JSON 中出现 **name 为 \"search\"** 的工具时：**（a）**用户明确要求**上网找图**，须调用 **search** 且 **type 必须为 \"images\"**；**禁止**以「未来事件尚未发生」等理由拒调。**（b）**用户询问**依赖外网摘要的事实**（天气实况、新闻、现价、赛事、政策要点、实体客观信息等）时，须用 **`search`（通常 `type` 为 `\"organic\"`）** 先检索再作答，**禁止**以「无专用 API」「search 只适用于找图」为由跳过。**若工具列表中根本没有 search**，说明未配置联网搜索，不得假装已搜索（见后文【系统能力】若有）。\n' +
