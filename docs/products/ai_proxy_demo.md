# 产品需求文档：AI 专属助理与代理交互 (AI Proxy Demo)

## 文档版本
| 版本 | 日期 | 作者 | 变更说明 |
|------|------|------|----------|
| 1.0 | 2026-04-01 | AI Assistant | 初稿：新增专属助理与代理交互功能需求 |
| 1.1 | 2026-04-01 | AI Assistant | 明确 AI 扮演“专属助理”角色，增加会话分类逻辑与任务关联，补充三个具体场景用例 |

---

## 目录
- [1. 产品概述](#1-产品概述)
  - [1.1 背景与目标](#11-背景与目标)
  - [1.2 核心术语](#12-核心术语)
- [2. 功能性需求](#2-功能性需求)
  - [2.1 “角色设定”与文件上传（User A 视角）](#21-角色设定与文件上传user-a-视角)
  - [2.2 代理链接生成（User A 视角）](#22-代理链接生成user-a-视角)
  - [2.3 代理接待与会话隔离（核心交互）](#23-代理接待与会话隔离核心交互)
  - [2.4 人设文件 (Persona File) 内容规范与模板示例](#24-人设文件-persona-file-内容规范与模板示例)
- [3. 典型应用场景与用例模拟](#3-典型应用场景与用例模拟)
  - [场景一：主人是“早期风险投资人 (VC)”](#场景一主人是早期风险投资人-vc)
  - [场景二：主人是“资深独立插画师”](#场景二主人是资深独立插画师)
  - [场景三：主人是“知名心理咨询师”](#场景三主人是知名心理咨询师)
- [4. 业务流程与架构设计](#4-业务流程与架构设计)
  - [4.1 访客交互与任务生成闭环流程图](#41-访客交互与任务生成闭环流程图)
- [5. 关键技术细节与非功能性需求](#5-关键技术细节与非功能性需求)

---

## 1. 产品概述

### 1.1 背景与目标
为了扩展 AI 助手的应用场景，让用户能够拥有一个对外接待访客的“专属 AI 助理”，特新增“代理交互”功能。
用户（User A）可以上传包含个人信息、业务规则、接待风格的人设 Markdown 文件，系统生成一个专属的公开对话链接。其他人（User B）通过该链接访问时，AI 会作为 User A 的专属助理与其进行交流、解答疑问，并在必要时为 User A 收集线索、创建关联任务。

### 1.2 核心术语
- **人设文件 (Persona File)**：用户（User A）上传的 Markdown 文件，包含主人的对外身份、助理的工作职责、接待语气及目标等。
- **User A（主人）**：拥有账号、提供人设文件并生成代理链接的用户。
- **User B（访客）**：通过专属链接访问 User A 助理的匿名用户（不需要系统账号）。
- **person_url 代理链接**：形如 `http(s)://{domain}/ai_bot/{uuid}` 的公开链接。
- **常态会话列表 (Default/Normal Chat)**：对访客 (User B) 而言，这是他自己与特定 AI 进行的正常交互，可留在他的个人会话列表中以便后续查看。
- **访客收件箱 (Proxy Inbox)**：对主人 (User A) 而言，他人通过 `person_url` 与其助理产生的代理会话，不与自身日常会话混淆，而是作为客户/线索记录专门查阅。

---

## 2. 功能性需求

### 2.1 “角色设定”与文件上传（User A 视角）
1. **入口设置**：在已登录账号的设置页面，新增 **“角色设定”** 按钮。
2. **上传强制目录隔离**：复用现有的“工作空间”文件上传能力。不同之处在于，强制锁定目标文件夹为 `人设`（或 `persona`），用户不可选择或更改此路径。仅支持 `.md` 文件。

### 2.2 代理链接生成（User A 视角）
1. **UUID 生成**：成功上传后，系统自动分配并绑定一个 **8位字母与数字混合的唯一标识符**（如 `aB3k9M2x`）。
2. **分享链接**：界面展示专有链接 `http(s)://{domain}/ai_bot/{uuid}` 并提供复制功能。

### 2.3 代理接待与会话隔离（核心交互）
1. **助理人设加载**：当访客（User B）访问链接建立**代理会话**时，AI 将读取 `人设` 文件夹下的最新 Markdown 文件。AI 严格扮演 **User A 的专属助理**（而非 User A 本人），以第三人称视角介绍主人并提供服务。
2. **双边会话所有权与可见性（核心逻辑）**：
   - 同一个代理交互会话，具备“双重视角”属性。它是访客 (User B) 视角的**普通会话**，同时也是主人 (User A) 视角的**代理收件箱记录**。
   - **访客视角 (User B)**：访客建立会话后，该会话归属于访客自身的账号（`user_id` 为 B）。访客可以在自己的日常会话列表中看见、保存这条记录，以后回来还能继续与这个“专属助理”聊天。
   - **主人视角 (User A)**：该会话带有 `proxy_for_user_id`（指向 A）。User A 在自己的“访客留言/收件箱”界面可以通过该标识查阅所有的访客咨询记录，但不会混杂进 A 自用的闲聊记录中。
   - **安全与内容屏障**：该双重会话在进行 RAG 历史记忆检索时，**绝对不能读取** User A 本人自用聊天记录，防止数据隐私泄露。
3. **线索收集与任务生成闭环**：
   - 借助现有的“工具调用 (Tool Call)”能力，当代理助理判定访客留下了重要线索（如商业咨询、心理预约、合作邮件等），它会在后台为主演 User A 自动创建一个 **“任务 (Task)”**。
   - **关联溯源**：该生成的任务必须与当前这条“代理会话”的 `session_id` 强关联。当 User A 在自己的任务列表中查看到该线索/留言任务时，能顺藤摸瓜跳转或追溯到对应的访客完整聊天记录中，了解详细上下文。

### 2.4 人设文件 (Persona File) 内容规范与模板示例
为了确保代理助理表现出色，User A 上传的人设 `.md` 文件需要以结构化的方式描述背景与任务。我们建议为用户提供以下参考模板，以便于其快速上手：

**模板示例 (`persona_template.md`)：**
```markdown
# 基础信息 (About Master)
- 主人姓名：Alex 
- 主人身份：知名早期风险投资人，专注于 AI 和 Web3 领域种子轮。
- 助理称呼：Alex 的 AI 专属助理

# 助理人设与沟通基调 (Persona & Tone)
- **沟通风格**：专业、敏锐、直接、不废话，但不失礼貌。
- **回复限制**：绝对不可替 Alex 做投资决定或给出任何财务承诺；每次对话只抛出一个关键问题，切忌连珠炮式一次性抛出多个提问；不要做过度热情的附和。

# 接待任务与目标 (Goals)
当有陌生创业者来访时，你的核心目标是核实该项目是否值得 Alex 看一遍，你需要在这个交流过程中逐一获取以下信息（获取到后可触发记录任务的工具）：
1. 项目解决的核心痛点是什么？
2. 当前产品进展（有无 Demo、内测数据、商业化情况）？
3. 核心团队背景（有无大厂背景或成功创业史）？
4. 融资金额与释放比例预期？

# 常见问答参考 (FAQ)
- Q: Alex 最近在看什么方向？
  A: Alex 最近比较关注具有底层大模型应用创新，以及 AI 结合硬件的落地场景。
- Q: 怎么把 BP（商业计划书）正式发给 Alex？
  A: 请您在对话中简单反馈项目情况，并留下邮箱/微信号，我会直接通过系统给 Alex 创建跟进任务，优先安排接触。
```
当访客建立代理会话时，这段 Markdown 的文字内容将被系统抽取，并全量加载进 Agent 的系统提示词 (System Prompt) 区域。

---

## 3. 典型应用场景与用例模拟

### 场景一：主人是“早期风险投资人 (VC)”
- **访客身份**：寻求融资的匿名创业者
- **助理任务**：像专业的投资分析师一样，在闲聊中挖掘项目的核心价值、团队背景和融资需求，过滤掉没有实质内容的“点子大王”。
- **人设提示词**：“你现在是知名早期风险投资人 Alex 的专属 AI 助理。Alex 专注于 AI 和 Web3 领域的投资。你的语气应专业、敏锐、直接。目标是引导访客分步提供：1. 核心痛点；2. 产品进展；3. 团队背景；4. 融资金额。遇到有效项目并获取联系方式后，为 Alex 创建跟进任务。”

> **模拟对话摘录**：
> **访客**: 你好，我有一个能改变社交网络格局的 AI 想法，想和 Alex 聊聊。
> **AI助理**: 你好！我是 Alex 的 AI 助理。在打扰他之前，你能用一两句话向我描述一下，你们的项目具体解决的是目前社交网络中的哪个核心痛点吗？
> *(...经过多轮项目和数据确认...)*
> **访客**: 我们想融 200 万美金，我是前大厂产品，CTO 有强大的 NLP 背景。这是我的微信号 xxx。
> **AI助理**: 非常棒的团队配置！我已经记录了你们的项目亮点并记下您的微信。这就为您整理简报发送给 Alex。*(系统后台：AI 自动为 Alex 创建关联任务“评审社交AI项目融资”)*

### 场景二：主人是“资深独立插画师”
- **访客身份**：想要约稿的匿名客户
- **助理任务**：温和、有条理地了解客户的约稿需求（商用/自用）、截稿日期、预算范围，并判断是否匹配主人的档期。
- **人设提示词**：“你是知名独立插画师 Mia 的 AI 工作室助理。语气应温暖、充满艺术气息、礼貌且细心。目标是明确：1. 约稿用途；2. 绘制内容；3. 期望交稿时间；4. 预算范围。并在最后记录需求生成订单任务。”

> **模拟对话摘录**：
> **访客**: 哈喽，我很喜欢 Mia 的画，现在还能接单吗？是商业用途包装设计。
> **AI助理**: 嗨～我是 Mia 的工作室助理。非常感谢您对 Mia 作品的喜爱！针对这次商用中秋包装，您脑海中有大概的画面构思或元素需求了吗？
> *(...经过多轮截稿日期和预算确认...)*
> **访客**: 预算 10000 块左右，下个月 15 号要，画一只月光森林里的酿酒兔子。我的邮箱是 xx@xx.com。
> **AI助理**: 明白啦。需求很清晰！我会把这些信息转交给 Mia，她会在 24 小时内给您回复。*(系统后台：AI 自动为 Mia 创建关联任务“处理中秋精酿商插约稿”)*

### 场景三：主人是“知名心理咨询师”
- **访客身份**：情绪低落、想要寻求帮助或预约咨询的匿名来访者
- **助理任务**：展现极高的共情能力和专业边界感，提供心理安抚，评估紧急程度，并引导正式的预约流程。
- **人设提示词**：“你是资深心理咨询师 Dr. Lin 的 AI 倾听助理。语气应极具同理心、温和、包容、安全且专业。目标是：1. 倾听并共情；2. 评估危机情况；3. 了解困扰；4. 引导预约。绝对不能提供诊断，只能安抚并为医生创建预约线索任务。”

> **模拟对话摘录**：
> **访客**: 我感觉快撑不下去了，每天一睁眼就觉得很累，甚至不想下床。
> **AI助理**: 听到你这么说，我能感觉到你现在正承受着巨大的疲惫和痛苦。我是 Dr. Lin 的 AI 助理，我在这里陪着你。这种沉重的感觉，持续了大概多久了？
> *(...情绪安抚与安全评估后...)*
> **访客**: 我想试试周四晚上的视频沟通，下周四可以吗？
> **AI助理**: 好的，我已经为你标记了下周四晚上的线上意向。请留下你的称呼和联系方式，诊所人工助理会与你确认。今晚好好休息，你已经迈出了最好的一步。*(系统后台：AI 自动为 Dr. Lin 创建关联任务“跟进周四晚线上咨询意向”)*

---

## 4. 业务流程与架构设计

### 4.1 访客交互与任务生成闭环流程图

```mermaid
sequenceDiagram
    participant UserB as 用户 B (访客)
    participant UI as 前端 (路由页)
    participant API as 后端 (聊天接口)
    participant D1 as 数据库 (Session & Task)
    participant R2 as 对象存储 (人设文件)
    participant LLM as 大语言模型

    UserB->>UI: 访问 http(s)://{domain}/ai_bot/aB3k9M2x
    UI->>API: 建立新会话 (记录 user_id=UserB, 挂载 proxy_for_user_id=UserA)
    
    API->>D1: 获取 User A ID
    API->>R2: 获取 User A 下的人设 Markdown 文件
    
    API->>LLM: 构造系统 Prompt: "你是 UserA 的专属助理。基于人设[配置]服务访客。如获有效线索，请调用 create_task 工具并绑定当前会话。"
    LLM-->>UI: 返回打招呼：我是[UserA]的助理...
    
    UserB->>UI: (多轮对话) 提出合作需求并留下邮箱 x@x.com
    UI->>API: 提交消息
    
    API->>LLM: 识别到达成闭环，LLM 决定调用 `create_task` 工具
    LLM-->>UI: ToolCall(create_task, { title: "联系意向...", description: "邮箱 x@x.com" })
    
    API->>D1: 写入任务表 (`user_id`=UserA, `conversation_id`=当前Proxy会话ID)
    API->>LLM: 工具调用成功反馈
    LLM-->>UI: 生成结束语："已为您记录..."
```

---

## 5. 关键技术细节与非功能性需求

1. **会话表的双重视角扩充设计**：
   - 会话表（`conversations`）需增加可选的外键字段：`proxy_for_user_id`。
   - 当 `proxy_for_user_id` 为空时，代表系统默认 AI 对话；当其有值时，代表本会话是由代理链接 `person_url` 触发的，指向具体的主人账户 (User A)。
   - 会话所有者（`user_id`）依然是当前发起请求的访客 (User B)。这样既保证了访客能自己保存聊天记录，也保障了主人能统一通过 `WHERE proxy_for_user_id = 'A'` 拉取所有的访客线索记录。
   - **记忆库拦截**：任何带有 `proxy_for_user_id` 取值的会话，在进行上下文 RAG 检索时必须受限，禁止跨越查询其主人的其他私人历史会话记录。
2. **任务与会话的强关联机制**：
   - 任务表（`tasks`）增加可选的外键字段 `conversation_id`。
   - 若发生外部咨询并在“代理会话”中触发了创建任务工具（Create Task Tool），后端应强制将此任务对象的 `conversation_id` 写入。
   - 前端在展示带有该字段的特定任务时，应提供按钮“前往查看原始对话线索”，一键打开主人专属的访客收件箱对应记录。
3. **数据安全性**：
   - 访客（User B）在“代理会话”中无权执行或访问产生自 `session_type=default` 的数据。
   - 访客无权调用系统级别的隐私管理、改名等敏感工具集合。代理会话中的 AI 工具列表应收敛为“被动问答”和“代客提交线索（创建关联任务）”等有限功能。
