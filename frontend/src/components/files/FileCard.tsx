...
function processedLabel(p: FileInfo['processed']): { text: string; className: string } {
  if (p === 1) {
    return {
      text: '已索引',
      className:
        'bg-emerald-100 text-emerald-900 ring-1 ring-emerald-500/50 dark:bg-emerald-950/55 dark:text-emerald-200 dark:ring-emerald-500/40',
    }
  }
  if (p === -1) {
    return {
      text: '处理失败',
      className:
        'bg-red-100 text-red-900 ring-1 ring-red-400/60 dark:bg-red-950/50 dark:text-red-200 dark:ring-red-500/35',
    }
  }
  return {
    text: '处理中',
    className:
      'bg-amber-100 text-amber-950 ring-1 ring-amber-500/45 dark:bg-amber-950/45 dark:text-amber-100 dark:ring-amber-500/35',
  }
}
  ...
  return (
    <div className="flex flex-wrap gap-2 md:justify-end">
      <button
        type="button"
        disabled={busy}
        className="rounded border border-slate-300 bg-white px-2 py-1 text-xs font-medium text-slate-800 hover:border-emerald-500/50 hover:text-emerald-900 disabled:opacity-50 dark:border-slate-600/80 dark:bg-slate-900/60 dark:text-slate-200 dark:hover:border-emerald-500/35 dark:hover:text-emerald-100"
      ...
      {file.processed === -1 && onRetryProcess ? (
        <button
          type="button"
          disabled={busy}
          className="rounded border border-amber-400/70 bg-amber-50 px-2 py-1 text-xs font-medium text-amber-950 hover:bg-amber-100/90 disabled:opacity-50 dark:border-amber-500/40 dark:bg-amber-950/45 dark:text-amber-100 dark:hover:bg-amber-950/65"
      ...
      <button
        type="button"
        disabled={busy}
        className="rounded border border-red-300/80 bg-red-50 px-2 py-1 text-xs font-medium text-red-800 hover:bg-red-100/90 disabled:opacity-50 dark:border-red-500/40 dark:bg-red-950/35 dark:text-red-200 dark:hover:bg-red-950/55"
    ...
    <article className={`relative overflow-hidden ${className}`}>
      {processing ? (
        <>
          ...
          />
          <div
            className="absolute left-0 right-0 top-0 z-[1] h-1.5 overflow-hidden rounded-t-[inherit] bg-slate-200 dark:bg-slate-950/90"
            role="progressbar"
          ...
          <div className="pointer-events-none absolute right-2 top-2 z-[1] rounded border border-amber-400/60 bg-amber-50 px-1.5 py-0.5 text-[10px] font-semibold tabular-nums tracking-tight text-amber-900 dark:border-amber-500/30 dark:bg-slate-950/85 dark:text-amber-100">
            {displayPct}%
          </div>
        </>
      ) : null}
      ...
      >
        <div className="flex min-w-0 flex-1 flex-col gap-3 md:flex-row md:gap-6">
          <div className="flex min-w-0 items-start justify-between gap-2 md:block md:w-56 md:shrink-0">
            <h3
              className="line-clamp-2 text-sm font-semibold text-slate-800 dark:text-slate-100"
