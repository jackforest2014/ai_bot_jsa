  ...
  return (
    <div className="border-t border-slate-200 bg-slate-50/90 px-3 py-2">
      <div className="mb-1 flex items-center justify-between gap-2">
        <span className="text-xs font-semibold text-slate-700">任务详情</span>
        {rawJson ? (
          <button
            type="button"
      ...
      {task.description ? (
        <p className="mb-2 text-xs text-slate-600 whitespace-pre-wrap">{task.description}</p>
      ) : (
        <p className="mb-2 text-xs text-slate-400">无描述</p>
      )}

      {subtasks && subtasks.length > 0 ? (
        <ul className="mb-2 list-inside list-decimal space-y-1 text-xs text-slate-800">
          {subtasks.map((item, i) => (
